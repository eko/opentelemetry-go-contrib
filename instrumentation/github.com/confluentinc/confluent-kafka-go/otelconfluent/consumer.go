...
import (
	...
	"github.com/eko/opentelemetry-go-contrib/instrumentation/github.com/confluentinc/confluent-kafka-go/otelconfluent/internal"
...
func (c *Consumer) startSpan(operationName internal.Operation, msg *kafka.Message) oteltrace.Span {
	...
	ctx := contextFromMessageHeaders(c.ctx, msg)

	_, span := c.tracer.Start(ctx, string(operationName), opts...)

	replaceOrAddSpanContextToMessageHeaders(span.SpanContext(), msg)
